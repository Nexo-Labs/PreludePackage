...
public extension UIWrapping {
    ...
    init(_ provider: @escaping RepresentableProvider<V>, update handler: (RepresentableUpdater<V>)? = nil) where V: UIView {
        wrapped = AnyView(
            ViewWrapper(
                provider: provider,
                updater: handler
    ...
    ///   - updateHandler: Use this closure to configure the view, this is called for all SwiftUI view updates as well
    init(_ type: V.Type = V.self, update handler: @escaping (V) -> ()) where V: UIView {
        wrapped = AnyView(
            ViewWrapper(
...
public extension UIWrapping {
    ...
    /// - Parameters:
    ///   - viewProvider: Use this closure to return an instantiated `UIView`
    ///   - updateHandler: Use this closure to configure the view, this is called for all SwiftUI view updates as
    
    init(_ provider: @escaping RepresentableProvider<V>, update handler: (RepresentableUpdater<V>)? = nil) where V: UIViewController {
        wrapped = AnyView(
    ...
    init(_ type: V.Type = V.self, update handler: @escaping (V) -> ()) where V: UIViewController {
        wrapped = AnyView(
            ViewControllerWrapper(
